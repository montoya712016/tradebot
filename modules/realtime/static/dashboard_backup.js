...
(function () {
  ...
  async function loadTradeDetail(info, updateSelection = true, force = false) {
    ...
    async function fetchWindow(endMsVal, lb) {
      return fetchJson(
        `/api/ohlc_window?symbol=${encodeURIComponent(info.symbol)}&end_ms=${endMsVal}&lookback_min=${lb}`
      );
    }
  ...
  async function loadSignalDetail(info, force = false) {
    ...
    async function fetchWindow(endMsVal, lb) {
      return fetchJson(
        `/api/ohlc_window?symbol=${encodeURIComponent(detail.symbol)}&end_ms=${endMsVal}&lookback_min=${lb}`
      );
    }
