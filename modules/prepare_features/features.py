...
# ===== Otimizações (Numba) para gargalos =====
@njit(cache=True)
def _dm_up_dn_numba(high: np.ndarray, low: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Directional Movement (+DM/-DM) no estilo Wilder, sem TR."""
    n = high.size
    up = np.empty(n, np.float64)
    dn = np.empty(n, np.float64)
    if n == 0:
        return up, dn
    up[0] = 0.0
    dn[0] = 0.0
...
def make_features(df: pd.DataFrame, flags: Dict[str, bool], *, verbose: bool = True) -> None:
    ...
    for idxs in df.groupby(seg_id).groups.values():
        ...
        if flags.get("adx"):
            _t0 = time.perf_counter()
            up, dn = _dm_up_dn_numba(high, low)
        ...
        if flags.get("time_since"):
            ...
            for m in MINMAX_MIN:
                ...
                df.loc[idxs, f"time_since_max_{m}"] = tmax
    ...
    quiet_env = os.getenv("PF_QUIET", "").strip().lower() in {"1", "true", "yes", "y", "on"}
    quiet_flag = False
    try:
        quiet_flag = bool(flags.get("quiet") or flags.get("_quiet"))
    except Exception:
        quiet_flag = False
    # Por padrão: não queremos spam de timings por feature em loops/batch.
    # Só imprime detalhamento se explicitamente solicitado via flag.
    show_timings = False
    try:
