...
4. **Exploração fair por steps**
   ...
   - o assistant agora suporta escolha de `model`, `reasoning` e modo de acesso do Codex remoto
   - quando uma conversa está aberta, novas mensagens continuam a mesma thread do `codex app-server`; use `Nova conversa` para abrir outra
...
Observações
-----------
- o `fair_dashboard.py` agora também expõe uma área administrativa `/assistant` para disparar prompts do Codex local no repositório; o acesso fica restrito a usuários admin.
...
- o histórico do assistant agora é orientado a conversa, não a job: cada novo envio continua a thread aberta e o backend usa `codex app-server` com `threadId` persistido quando já existe `session_id` anterior.
...
- no modo `reuse`, o OOS agora roda somente no bloco seguinte ao backtest IS do step fonte; a costura padrao da `v6` vai de `1440 -> 1260` ate `540 -> 360`, sem reutilizar o mesmo bloco do explore.
