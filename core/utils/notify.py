...
Permite envio de alertas para celular/desktop via API Pushover.
"""
...
def load_from_local_secrets(
    *,
    ...
    """
    Fallback local: lê `config/secrets.py` (se existir).
    """
    try:
        # Tenta importar de config.secrets (assumindo que config está no path)
...
def load_default(
    *,
    ...
    """
    Carrega configuração padrão (ENV > Secrets).
    """
