# -*- coding: utf-8 -*-
"""
Utilitários de caminhos do projeto.

Centraliza a lógica de diretórios do repositório, workspace e dados externos.
"""
...
def modules_root() -> Path:
    ...
    # Ajuste: repo_root/modules
    return repo_root() / "modules"


def repo_root() -> Path:
    """Retorna a raiz do repositório (onde fica `core/`, `modules/`, etc)."""
    # core/utils/paths.py -> utils -> core -> repo_root
    return Path(__file__).resolve().parents[2]


def workspace_root() -> Path:
    """
    Retorna a raiz do workspace (pai do repo).
    Geralmente onde ficam pastas grandes como `models_sniper/` e `cache_sniper/`.
    """
    return repo_root().parent


def models_root() -> Path:
    """Raiz para modelos salvos."""
    return workspace_root() / "models_sniper"


def models_root_for_asset(asset_class: str | None = None) -> Path:
    """Raiz para modelos de um ativo específico (crypto/stocks)."""
...
def cache_sniper_root() -> Path:
    """Raiz do cache do Sniper (dados brutos, features)."""
...
def feature_cache_root() -> Path:
    """Raiz do cache de features calculadas."""
...
def resolve_repo_path(p: PathLike) -> Path:
    """
    Resolve caminhos relativos usando a raiz do repo.
    """
    pp = Path(p)
    if pp.is_absolute():
        return pp
